...
def add_time_features(df):
    ...
    week_cols = [
        weekdays[x] if x is not None and 0 <= x < 7 else None
        for x in df["DayOfWeekNum"]
    ]
    df = df.with_columns([pl.Series("DayOfWeek", week_cols)]).drop("DayOfWeekNum")
    return df
...
def create_flags(df):
    ...
    df = df.with_columns(
        [
            ...
            pl.col("Cancelled Rides by Customer")
            .is_not_null()
            .alias("Is_Cancelled_Customer"),
            pl.col("Cancelled Rides by Driver")
            .is_not_null()
            .alias("Is_Cancelled_Driver"),
        ]
    )
    df = df.with_columns(
        [
            pl.when(pl.col("Booking Status") == "Completed")
            .then(pl.lit("Completed"))
            .when(pl.col("Booking Status").str.contains("Cancelled"))
            .then(pl.lit("Cancelled"))
            .when(pl.col("Booking Status") == "No Driver Found")
            .then(pl.lit("No Driver Found"))
...
def basic_metrics(df):
    ...
    print(
        f"-> Successful rides: {successful_rides} ({successful_rides*100/total_rides:.1f}%)"
    )
...
def plot_success_patterns(df):
    ...
    vehicle_success = pd.DataFrame(
        vehicle_success_data,
        columns=["Vehicle Type", "Total_Rides", "Successful_Rides", "Success_Rate"],
    ).sort_values("Success_Rate", ascending=False)
    colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728"]
    axes[0, 0].bar(
        vehicle_success["Vehicle Type"],
        vehicle_success["Success_Rate"] * 100,
        color=colors,
    )
    ...
    hourly_success = pd.DataFrame(
        hourly_success_data,
        columns=["Hour", "Total_Rides", "Successful_Rides", "Success_Rate"],
    )
    ...
    daily_success = pd.DataFrame(
        daily_success_data,
        columns=["DayOfWeek", "Total_Rides", "Successful_Rides", "Success_Rate"],
    )
    colors = [
        "#377eb8",
        "#4daf4a",
    ...
    axes[1, 0].bar(
        daily_success["DayOfWeek"], daily_success["Success_Rate"] * 100, color=colors
    )
    ...
    status_counts = pd.DataFrame(
        status_counts_data, columns=["Status_Category", "Count"]
    )
    axes[1, 1].pie(
        status_counts["Count"],
        labels=status_counts["Status_Category"],
        autopct="%1.1f%%",
    )
...
def plot_revenue_analysis(df):
    ...
    vehicle_revenue_data = [
        (
            ...
            rev,
            avg,
            (rev / total_revenue_all * 100 if total_revenue_all > 0 else 0),
        )
    ...
    colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728"]
    axes[0, 0].bar(
        vehicle_revenue["Vehicle Type"], vehicle_revenue["Total_Revenue"], color=colors
    )
...
def revenue_prediction_model(df):
    ...
    df_revenue_model = df.filter(
        pl.col("Is_Successful") & pl.col("Booking Value").is_not_null()
    )
    ...
    df_model_features = df_revenue_model.with_columns(
        [
            pl.col("Hour")
            .map_elements(lambda x: np.sin(2 * np.pi * x / 24), return_dtype=pl.Float64)
            .alias("Hour_Sin"),
            pl.col("Hour")
            .map_elements(lambda x: np.cos(2 * np.pi * x / 24), return_dtype=pl.Float64)
            .alias("Hour_Cos"),
            pl.col("Month")
            .map_elements(lambda x: np.sin(2 * np.pi * x / 12), return_dtype=pl.Float64)
            .alias("Month_Sin"),
            pl.col("Month")
            .map_elements(lambda x: np.cos(2 * np.pi * x / 12), return_dtype=pl.Float64)
            .alias("Month_Cos"),
    ...
    df_model_features = df_model_features.with_columns(
        [
            pl.Series(
                ...
                le_vehicle.fit_transform(df_model_features["Vehicle Type"].to_list()),
            ),
            pl.Series(
                "Pickup_Encoded",
                le_pickup.fit_transform(df_model_features["Pickup Location"].to_list()),
            ),
            pl.Series(
                "Drop_Encoded",
                le_drop.fit_transform(df_model_features["Drop Location"].to_list()),
            ),
        ]
    )
    feature_columns = [
    ...
    revenue_feature_importance = pd.DataFrame(
        {"Feature": feature_columns, "Importance": rf_revenue.feature_importances_}
    ).sort_values("Importance", ascending=False)
    plt.figure(figsize=(8, 5))
    sns.barplot(
        x="Importance",
        y="Feature",
        data=revenue_feature_importance,
        orient="h",
        color="red",
    )
